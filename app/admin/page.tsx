...
export default function AdminPage() {
  ...
  const sales = todayOrders.filter(o => o.payment_status === "paid").reduce((sum,o) => sum + Number(o.total), 0);
  const open = orders.filter(o => !["completed","cancelled"].includes(o.order_status));
  ...
  return <main className="admin-page"><div className="admin-shell">
    ...
    <section className="orders-list"><h2>Orders</h2>{orders.length===0?<div className="empty">No orders yet.</div>:orders.map(order=><article className="order-card" key={order.id}>
      ...
      <div className="order-bottom"><span className={order.payment_status === "paid" ? "paid" : "pending"}>{order.payment_status.toUpperCase()} {order.payment_method ? `· ${order.payment_method}` : ""}</span><select value={order.order_status} onChange={e=>updateStatus(order.id,e.target.value)}>{statuses.map(s=><option key={s}>{s}</option>)}</select><button onClick={()=>window.print()}>Print receipt</button></div>
    </article>)}</section>
  </div></main>;
}
